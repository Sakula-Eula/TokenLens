...
const chart = computed(() => {
  ...
  const values = items.map((item) => Number(item.total_tokens) || 0);
  ...
  const points = values.map((value, index) => {
    ...
    const y = bottom - value / axisMax * (bottom - top);
    return { x, y, value, item: items[index] };
  });
  const linePath = points.map((point, index) => `${index ? "L" : "M"}${point.x.toFixed(1)},${point.y.toFixed(1)}`).join(" ");
  const areaPath = points.length ? `${linePath} L${points.at(-1).x.toFixed(1)},${bottom} L${points[0].x.toFixed(1)},${bottom} Z` : "";
  ...
  return {
    ...
    linePath,
    areaPath,
    labels: labelIndexes.map((index) => ({ x: points[index]?.x || left, text: formatBucket(items[index]?.bucket, index === items.length - 1) })),
  };
});
...
<template>
  <div class="tray-page" @mouseenter="reportPointer(true)" @mouseleave="reportPointer(false)">
    ...
    <section class="trend-card card">
      ...
      <svg class="trend-chart" viewBox="0 0 435 122" role="img" aria-label="Token 使用趋势折线图">
        <defs><linearGradient id="trend-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#4f91ef" stop-opacity=".28" /><stop offset="1" stop-color="#4f91ef" stop-opacity=".02" /></linearGradient></defs>
        ...
        <path v-if="chart.areaPath" :d="chart.areaPath" fill="url(#trend-fill)" />
        <path v-if="chart.linePath" :d="chart.linePath" class="trend-line" />
        <circle v-for="(point, index) in chart.points" :key="index" :cx="point.x" :cy="point.y" r="2.3" />
