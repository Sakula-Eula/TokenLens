...
import { POLL_INTERVAL_MS, fetchErrors, fetchRequests } from "../api";
...
async function refresh() {
  ...
  try {
    const [statsData, requestsData] = await Promise.all([fetchErrors(props.range), fetchRequests(params)]);
...
<template>
  <div class="errors-page" :class="{ loading }">
    ...
    <div v-if="loadError" class="error-banner"><AppIcon name="alert" :size="17" />{{ loadError }}</div>
    ...
    <section class="panel request-panel">
      <div class="filters">
        ...
        <label><span>状态码</span><input v-model="filters.status" type="number" placeholder="例如 429" /></label>
      ...
      <div class="table-wrap"><table>
        ...
        <tbody>
          <tr v-for="item in data.items" :key="item.id"><td>{{ formatTime(item.created_at) }}</td><td>{{ item.provider }}</td><td>{{ item.model || 'unknown' }}</td><td class="mono">{{ item.request_id }}</td><td class="mono">{{ item.endpoint }}</td><td>{{ item.stream ? '流式' : '非流式' }}</td><td><span class="status">{{ item.status_code }}</span></td><td>{{ item.error_type || 'unknown' }}</td><td>{{ (item.latency_ms / 1000).toFixed(1) }}s</td></tr>
