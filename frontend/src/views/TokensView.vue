...
const cards = computed(() => [
  ...
  ["Cache Write", summary.value.cache_write_tokens], ["Cache Total", summary.value.cache_tokens],
]);
function fmt(value) { return (Number(value) || 0).toLocaleString("zh-CN"); }
function updateChart() { chart?.setOption({ color: ["#2476f5", "#20b77a", "#8b5cf6", "#f79009", "#ef4444"], tooltip: { trigger: "axis" }, legend: { bottom: 0, data: ["Total", "Input", "Output", "Cache Read", "Cache Write"] }, grid: { left: 20, right: 20, top: 25, bottom: 45, containLabel: true }, xAxis: { type: "category", boundaryGap: false, data: trend.value.map(item => item.bucket.slice(5).replace("T", " ")), axisLabel: { hideOverlap: true, color: "#7b879b" } }, yAxis: { type: "value", axisLabel: { color: "#7b879b" }, splitLine: { lineStyle: { color: "#edf1f6", type: "dashed" } } }, series: [["Total", "total_tokens"], ["Input", "input_tokens"], ["Output", "output_tokens"], ["Cache Read", "cache_read_tokens"], ["Cache Write", "cache_write_tokens"]].map(([name, key]) => ({ name, type: "line", smooth: true, showSymbol: false, data: trend.value.map(item => item[key]) })) }, true); }
