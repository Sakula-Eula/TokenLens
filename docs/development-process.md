...
## 2026-08-23：悬浮窗指标卡改为图标置顶

### 背景

悬浮窗顶部的四项指标卡原先以图标在左、数值在右的横向方式展示。用户要求将图标移至数值上方。

### 需求澄清

- 仅调整悬浮窗顶部四项指标卡的内部排列。
- 保留卡片数量、数值、颜色、边框和整体尺寸。

### 任务拆分

| 阶段 | 任务 | 结果 |
| --- | --- | --- |
| 前端 | 将指标卡内容改为居中的纵向排列 | 已完成 |
| 验证 | 执行 Vue 生产构建和差异检查 | 通过 |

### 设计与实施

`frontend/src/views/TrayOverviewView.vue` 为 `.metrics article` 增加纵向 flex 布局与居中对齐，使图标显示在数值上方；各指标原有的图标尺寸、配色、数值格式和卡片高度保持不变。

### 交互边界

- 此调整只影响托盘悬浮窗的四项概览指标卡，不影响 Dashboard 或其他统计卡片。

### 验证记录

- Vue 构建：在 `frontend/` 执行 `npm run build`，Vite 成功转换 681 个模块并生成生产资源。
- `git diff --check`：通过；仅出现 Git 的既有 LF/CRLF 转换提示。
- 构建仍提示现有共享包压缩后超过 500 kB，与本次布局调整无关。

### 已知限制

- 需要重新打开或刷新悬浮窗实例，才能看到新的布局。
## 2026-08-23：Provider 保存成功浮动提示

### 背景

Provider 配置保存成功的既有提示位于设置页顶部；用户在页面底部点击“保存设置”时，提示容易落在视口外，因而没有可见反馈。

### 需求澄清

- Provider 保存成功后应提供即时、可见的成功消息。
- 提示不影响现有配置保存接口和失败提示。
...
| 阶段 | 任务 | 结果 |
| --- | --- | --- |
| 前端 | 将设置页消息改为右下角浮动提示并增加自动关闭 | 已完成 |
| 验证 | 执行 Vue 生产构建和差异检查 | 通过 |

### 设计与实施

`frontend/src/views/SettingsView.vue` 新增 `showSuccess()`：成功保存 Provider 配置后在右下角显示成功消息，3.5 秒后自动关闭；复制 Harness Base URL 也复用该消息入口。消息使用 `role="status"`，错误消息使用 `role="alert"`，并在组件卸载时清理计时器。

### 交互边界

- 仅影响设置页的成功和失败消息展示位置；Provider 保存的数据、接口和立即生效逻辑不变。
- 成功消息在新的成功操作发生时重新计时；失败消息仍持续显示，直至后续操作清除或替换。

### 验证记录

- Vue 构建：在 `frontend/` 执行 `npm run build`，Vite 成功转换 681 个模块并生成生产资源。
- `git diff --check`：通过；仅出现 Git 的既有 LF/CRLF 转换提示。
- 构建仍提示现有共享包压缩后超过 500 kB，与本次提示改动无关。

### 已知限制

- 当前未配置 Vue 组件交互测试；本次通过生产构建验证。
## 2026-08-23：请求记录单条与筛选批量删除

### 背景

用户需要从请求记录中清理不再需要的数据。原系统只提供查询和详情接口，无法删除单条请求或按筛选条件清理多条请求。

### 需求澄清

- 支持删除单条请求记录。
- 支持按当前 Provider、模型、状态或日期筛选条件批量删除。
- 删除操作不可撤销，且必须避免未筛选时删除全部历史数据。

### 任务拆分

| 阶段 | 任务 | 结果 |
| --- | --- | --- |
| 数据库 | 删除请求及其关联费用快照，并限制批量删除必须有筛选条件 | 已完成 |
| API | 提供单条和筛选批量删除接口 | 已完成 |
| 前端 | 增加行级删除、批量删除和二次确认 | 已完成 |
| 验证 | 执行后端测试、Vue 构建和差异检查 | 通过 |

### 设计与实施

`backend/database/queries.py` 新增 `delete_request()` 和 `delete_requests()`。二者均在同一数据库事务中先删除 `request_costs` 费用快照，再删除 `api_requests` 请求记录；批量删除若未提供任何筛选条件会抛出错误。

`backend/api/requests.py` 新增 `DELETE /api/requests/{record_id}` 和 `DELETE /api/requests`。后者复用请求列表的 Provider、模型、状态与日期筛选参数，返回实际删除数量。

`frontend/src/views/RequestsView.vue` 为请求表格加入单条“删除”按钮；设置任一筛选条件后显示“删除筛选结果（数量）”按钮。两种操作都使用浏览器确认框说明不可撤销，批量删除还明确提示关联费用快照会被一并删除；完成后刷新列表和统计。

### 交互边界

- 无筛选条件时不会显示批量删除入口；即使直接调用接口，后端也会返回 HTTP 400。
- 删除单条不存在的记录返回 HTTP 404。
- 已删除请求不会从统计、趋势、费用汇总或请求详情中继续出现。

### 验证记录

- 后端测试：执行 `.\.venv\Scripts\python.exe -m pytest tests\test_requests_api.py tests\test_database.py -q`，结果为 `12 passed in 0.85s`；pytest 输出一条既有的 `asyncio_default_fixture_loop_scope` 弃用警告。
- Vue 构建：在 `frontend/` 执行 `npm run build`，Vite 成功转换 681 个模块并生成生产资源。
- `git diff --check`：通过；仅出现 Git 的既有 LF/CRLF 转换提示。
- 构建仍提示现有共享包压缩后超过 500 kB，与本次删除功能无关。

### 已知限制

- 当前批量删除以当前所有筛选结果为范围，不提供跨页勾选后只删除部分记录的功能。
## 2026-08-23：悬浮窗趋势改为极简单色柱并调整周期

### 背景

用户提供了使用趋势的视觉参考，要求采用细窄的浅蓝色单色柱状火花线，并将悬浮窗的时间范围调整为 `12H / 今日 / 7D`。

### 需求澄清

- 移除原有 `1H` 与 `6H` 范围，保留“今日”和 `7D`，新增滚动 `12H`。
- 趋势仅展示总 Token，不再区分缓存命中、未命中输入和输出，也不显示坐标轴、网格或图例。
- `今日` 继续使用既有 `24h` API 参数，语义保持为本地当天零点至当前时刻。

### 任务拆分

| 阶段 | 任务 | 结果 |
| --- | --- | --- |
| 后端 | 扩展统计接口的 `12h` 范围并按小时聚合 | 已完成 |
| 前端 | 替换范围切换为 `12H / 今日 / 7D` | 已完成 |
| 视觉 | 改为紧凑浅蓝色细柱火花线 | 已完成 |
| 验证 | 运行统计测试、前端生产构建与差异检查 | 通过 |

### 设计与实施

`backend/database/queries.py` 将 `12h` 纳入统计范围，其起点为当前时刻向前推 12 小时；趋势仍按小时桶返回。`backend/api/stats.py` 同步接受 `12h` 参数。

`frontend/src/views/TrayOverviewView.vue` 直接按所选范围请求趋势数据，取消此前仅在前端截取 `1H` 和 `6H` 数据的逻辑。SVG 趋势图以每个时间桶的 `total_tokens` 绘制浅蓝色细柱，移除了分段颜色、坐标轴、网格和图例，并缩短趋势卡高度以贴近参考图的火花线布局。

### 交互边界

- `12H` 为滚动时间窗口；“今日”从本地当天零点开始；`7D` 继续为滚动 7 天。
- 柱状图保留原生 SVG 悬浮提示，显示该时间桶及总 Token 数。
- 模型使用量区块和其他悬浮窗指标不受本次图表简化影响。

### 验证记录

- 范围校验：执行 `./.venv/Scripts/python.exe -c "from backend.database.queries import range_since; assert range_since('12h') > range_since('last24h')"`，通过。
- 后端测试：执行 `./.venv/Scripts/python.exe -m pytest tests/test_stats_api.py tests/test_database.py`，结果为 `15 passed in 0.93s`；pytest 输出一条既有的 `asyncio_default_fixture_loop_scope` 弃用警告。
- Vue 构建：在 `frontend/` 执行 `npm run build`，Vite 成功转换 681 个模块并生成生产资源；保留既有共享包大于 500 kB 的警告。
- `git diff --check`：通过；仅显示 Git 的 LF/CRLF 转换提示。

### 已知限制

- 趋势按有请求的时间桶绘制，不补齐空桶；低频使用时柱间距会较大。
- 当前未配置悬浮窗的组件级视觉回归测试，本次以生产构建和 SVG 源码审查验证。
## 2026-08-23：请求删除自定义确认弹窗

### 背景

请求删除原先通过浏览器原生确认框完成，样式与 TokenLens 界面不一致，且无法清晰呈现单条或批量删除的影响范围。

### 需求澄清

- 用项目内的确认界面替换浏览器原生确认框。
- 单条删除显示请求标识，批量删除显示受影响记录数。
- 明确说明费用快照会被同步删除且操作不可撤销。

### 任务拆分

| 阶段 | 任务 | 结果 |
| --- | --- | --- |
| 前端 | 新增删除确认模态框并接入单条和批量删除入口 | 已完成 |
| 交互 | 提供遮罩、关闭、取消、确认和执行中禁用状态 | 已完成 |
| 验证 | 执行 Vue 生产构建和差异检查 | 通过 |

### 设计与实施

`frontend/src/views/RequestsView.vue` 以 `pendingDeletion` 保存等待确认的单条或批量删除上下文。点击删除时仅打开 `Teleport` 渲染的自定义模态框；用户确认后才调用既有删除接口。弹窗使用警示图标、删除对象/数量、不可撤销说明以及区分视觉层级的取消和红色确认按钮。

### 交互边界

- 可通过遮罩、右上角关闭按钮或“取消”退出确认，不会删除数据。
- 请求执行期间关闭、取消和确认按钮均禁用，避免重复提交。
- 删除接口、筛选限制和关联费用快照清理逻辑保持不变。

### 验证记录

- Vue 构建：在 `frontend/` 执行 `npm run build`，Vite 成功转换 681 个模块并生成生产资源。
- `git diff --check`：通过；仅出现 Git 的既有 LF/CRLF 转换提示。
- 构建仍提示现有共享包压缩后超过 500 kB，与本次确认弹窗改动无关。

### 已知限制

- 当前未配置 Vue 组件交互测试；本次通过生产构建验证。
## 2026-08-24：悬浮窗趋势周期与聚合粒度调整

### 背景

用户要求将悬浮窗 Token 趋势的时间范围明确调整为 `12H / 24H / 7D`，并让短周期按小时、7 天周期按三小时展示，以兼顾近期细节和较长时间段的可读性。

### 需求澄清

- 悬浮窗只显示 `12H`、`24H`、`7D` 三个趋势选项。
- `12H` 和 `24H` 均按一小时聚合。
- `7D` 按三小时聚合。
- 不改变其他页面既有趋势接口默认的聚合方式。

### 任务拆分

| 阶段 | 任务 | 结果 |
| --- | --- | --- |
| 悬浮窗 | 更新周期标签并传递分桶粒度 | 已完成 |
| 统计接口 | 支持可选的 1 小时或 3 小时分桶参数 | 已完成 |
| 数据库 | 增加三小时分桶 SQL 聚合 | 已完成 |
| 测试 | 覆盖三小时聚合、后端统计及前端构建 | 通过 |

### 设计与实施

`frontend/src/views/TrayOverviewView.vue` 将中间选项标签由“今日”更新为 `24H`。加载趋势时，`12H` 和 `24H` 传递 `bucket_hours=1`，`7D` 传递 `bucket_hours=3`。

`backend/api/stats.py`、`backend/statistics/service.py` 与 `backend/database/queries.py` 为趋势接口增加可选 `bucket_hours` 参数，仅接受 `1` 或 `3`。三小时分桶把每条记录归入当天的 `00/03/06/09/12/15/18/21` 起始小时；未指定参数的既有调用继续保留原聚合行为，避免影响 Dashboard 和分析页。

`tests/test_database.py` 新增三小时分桶测试，验证同一三小时窗口内的 Token 会相加，并在下一个窗口开始新的聚合桶。

### 交互边界

- `12H` 是滚动 12 小时，`24H` 继续表示本地当天零点至当前时刻，`7D` 是滚动 7 天。
- 悬浮窗仍只绘制存在请求的时间桶，不补齐零值桶。
- API 未指定 `bucket_hours` 时保持此前按范围决定的默认分桶；该自定义参数不会改变其他页面的现有趋势展示。

### 验证记录

- 后端测试：执行 `pytest tests/test_database.py tests/test_stats_api.py`，结果为 `16 passed in 7.45s`。
- Vue 构建：执行 `npm run build --prefix frontend`，Vite 成功转换 681 个模块并生成生产资源；保留现有共享包大于 500 kB 的警告。
- `git diff --check`：通过；仅出现 Git 的 LF/CRLF 转换提示。

### 已知限制

- 当前仅支持 1 小时和 3 小时两种自定义趋势分桶粒度；如后续需要其他粒度，需要同时扩展接口参数校验与数据库聚合逻辑。
- 当前未配置悬浮窗的组件级视觉回归测试，本次通过生产构建、后端统计测试和源代码审查验证。
## 2026-08-25：本地代理密钥与管理接口加固

### 背景

开源前安全审查发现，配置 API Key 的自动兜底会允许任意本机调用消耗额度，且本地管理写接口没有独立访问控制。

### 设计与实施

`backend/config.py` 新增顶层 `allow_api_key_fallback` 配置，默认值为 `false`。代理仅在用户明确开启该选项时才使用 Provider 配置中的 API Key；否则客户端必须自行携带认证头。

应用启动时生成随机管理令牌并仅保存在内存。`backend/admin_auth.py` 使用常量时间比较校验 `X-TokenLens-Admin`；应用中间件保护所有 `/api/` 的 POST、PUT、PATCH 和 DELETE 请求。桌面托盘将令牌传给 WebView 首次地址，前端读取后立即从地址栏移除，并仅在写请求中发送该请求头。

### 验证记录

- 执行 Python 语法检查：通过。
- 验证默认不注入 Key、显式开启后才注入：通过。
- `git diff --check`：通过；仅有既有 LF/CRLF 提示。
- 相关既有测试仍按旧的默认兜底和未授权写入行为断言，尚未完成同步更新。

### 已知限制

- 此机制防护 localhost 误调用和普通本机程序滥用，无法防御已获得同一 Windows 用户完全控制权的恶意软件。
- 命令行直接启动 Uvicorn 时不会获得桌面令牌，管理写接口会返回 401。
